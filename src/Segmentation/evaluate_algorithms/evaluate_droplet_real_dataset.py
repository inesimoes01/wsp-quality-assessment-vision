...
def main_real_ccv(path_statistics, path_segmentation, fieldnames_statistics, fieldnames_segmentation, path_dataset, path_dataset_results):
    dataset_image_path = os.path.join(path_dataset, config.DATA_GENERAL_IMAGE_FOLDER_NAME)
    dataset_yolo_label_path = os.path.join(path_dataset, config.DATA_GENERAL_LABEL_FOLDER_NAME)
    ...
    for file in os.listdir(dataset_image_path):
        ...
        predicted_droplets, droplet_shapes, width, height, predicted_stats = apply_ccv_segmentation(im, im_gray, filename, path_dataset_results)
        seg_time = time.time()

        # get groundtruth
        groundtruth_polygons_with_centroid = get_yolo_polygons_from_label(os.path.join(dataset_yolo_label_path, filename + ".txt"), width, height)
