...
def draw_three_layer_circle_big(img, center, radius, inner_colors, middle_colors, outer_colors, background_c):
...
def draw_three_layer_circle_small(img, center, radius, inner_colors, middle_colors, outer_colors):
    for y in range(center[1] - radius, center[1] + radius):
        for x in range(center[0] - radius, center[0] + radius):
            if (x - center[0])**2 + (y - center[1])**2 <= radius**2:
                # calculate distance from center normalized to [0, 1]
                ...
                if distance < 0.8:  # middle part (60% to 70% of the radius)
                    t = distance / 0.8  # Scale t to [0, 1]
