...
class Segmentation:
                ...
                match segmentation_method:
                    case 0: # hough circles with clustering
                        ...
                        for j, (contour, roi_mask, roi_mask_filled) in enumerate(zip(final_contours, roi_mask_list, roi_mask_filled_list)):
                            ...
                            # check for the shape again, in case the droplets got separeted with the harsher threshold
                            shape, no_droplets, isOnEdge = self.check_for_contour_shape(contour, contour_area, x_rect, y_rect, w_rect, h_rect)                
    ...
    def get_contours(self):        
        ...
        #     self.contour_image = cv2.cvtColor(self.contour_image, cv2.COLOR_BGR2RGB)
        #     cv2.imwrite(os.path.join(config.RESULTS_LATEX_PIP_DIR, self.filename + "contours.png"), self.contour_image)
