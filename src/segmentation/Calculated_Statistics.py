...
class Calculated_Statistics:
    ...
    def __init__(self, image, filename, save_images:bool, create_masks:bool):
        ...
        for i, contour in enumerate(self.contours): 
            ...
            # check the shape to categorize it into overlapped/ellipse or single
            isOverlapped = self.check_for_shape(contour)

            # crop ROI of the droplet to analyze
            if self.save_images:
                file, img, x_roi, y_roi = self.crop_ROI(contour, isOverlapped, i)
            else: 
                img, x_roi, y_roi = self.crop_ROI(contour, isOverlapped, i)
                file = None
            ...
            if isOverlapped: 
                ...
                # save each circle detected. if no circles detected, we assume its an ellipse
    ...
    def crop_ROI(self, contour, isOverlapped, index):
        ...
        if self.save_images:
            if (isOverlapped): 
                path = os.path.join(self.path_to_save_contours_overlapped, str(index) + '.png')
                cv2.imwrite(path, object_roi)
                return path, object_roi, x, y
            else: 
                path = os.path.join(self.path_to_save_contours_single, str(index) + '.png')
                cv2.imwrite(path, object_roi)
                return path, object_roi, x, y
        else: 
            return object_roi, x, y
